...
func must_find_node(nodeInSceneTree: Node,path: String):
	...
	if node == null:
		PG_SceneTree.panic(nodeInSceneTree,"not found node by path %s" % path)
	return node
...
func must_change_scene(node: Node,scene_path:String):
	...
	if err_code != OK :
		PG_SceneTree.panic(node,"Error: change scene to %s, err code: %s" % [scene_path,err_code])
...
func must_connect_signal(nodeInTree:Node,signal_owner:Node,signal_name: String,method_owner:Object,method_name:String, binds:Array=[]):
	...
	if err_code != OK:
		PG_SceneTree.panic(nodeInTree,"failed to connect signal %s" % err_code)
		return

func must_instance_scene_from_path(nodeInTree: Node,scene_path: String):
	var scene_resource  = load(scene_path)
	if (scene_resource == null):
		PG_SceneTree.panic(nodeInTree,"failed to load scene from path %s" % scene_path)
